...
DLL_PATH="C:\\Program Files\\Analog Devices\\USB Drivers\\ADI_CYUSB_USB4.dll"

class ad5372:
	""" Class to control the AD537x EVAL board. """

	def __init__(self, vid=1110, pid=45583):
	...
	def Connect_to_Board(self, num_boards=1):
		...
		ret_init = self.Initialise()
		if ret_search != 0 or ret_conn != 0 or ret_firm != 0 or ret_init != 0:
			return 1
		else:
			return 0
	...
	def Write_SPI_Word(self, word):
		...
		ret = self.dll.Vendor_Request(self.handle, request, c_value, c_index, c_direction, c_zero, byref(c_buffer))
		return ret

	def Pulse_LDAC(self):
		""" Pulse LDAC. """
		c_req = c_uint(int('DE', 16))
		c_zero = c_short(0)
		c_buffer = create_string_buffer("",0)
		c_direction = c_char('0')
	...
	def Initialise(self):
		""" Call initialise function after the firmware has been downloaded.
		Thanks dpersechini for pointing this out. """
		request = c_ubyte(int('E0', 16))
		c_zero = c_short(0)
		c_direction = c_char('0')
		c_buffer = create_string_buffer("",0)
		ret = self.dll.Vendor_Request(self.handle, request, c_zero, c_zero, c_direction, c_zero, byref(c_buffer))
		return ret
